...
import { NICHES, Niche } from './config';
import { RateLimitError, TimeoutError, AuthError } from './errors';

const TREND_CACHE_TTL = 15 * 60 * 1000; // 15 minutes
const cache = new Map<string, { trends: Trend[]; timestamp: number }>();

async function fetchRedditTrends(niche: Niche): Promise<Trend[]> {
  ...
  try {
    ...
    for (const subreddit of niche.subreddits.slice(0, 2)) {
      ...
      for (const post of response.data.data.children) {
        ...
        const hasKeyword = niche.keywords.some(
    ...
    if (axios.isAxiosError(error)) {
      if (error.response?.status === 429) {
        throw new RateLimitError('Reddit rate limit exceeded');
      }
      if (error.response?.status === 401) {
        throw new AuthError('Reddit authentication failed');
      }
      if (error.code === 'ECONNABORTED') {
        throw new TimeoutError('Reddit request timed out');
      }
    }
...
async function fetchYouTubeTrends(niche: Niche): Promise<Trend[]> {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response?.status === 403) {
        throw new AuthError('YouTube API key invalid');
      }
      if (error.code === 'ECONNABORTED') {
        throw new TimeoutError('YouTube request timed out');
      }
    }
...
async function fetchNewsTrends(niche: Niche): Promise<Trend[]> {
  const trends: Trend[] = [];
...
export async function discoverTrends(
  ...
  if (newsTrends.status === 'fulfilled') allTrends.push(...newsTrends.value);

  // Sort by source priority: reddit > youtube > news
  const sourceOrder: Record<string, number> = { reddit: 0, youtube: 1, news: 2 };
  allTrends.sort((a, b) => sourceOrder[a.source] - sourceOrder[b.source]);

  // Deduplicate by title similarity
  const uniqueTrends: Trend[] = [];
...
export function clearTrendCache(nicheId?: string): void {
  if (nicheId) {
...
export const getTrends = discoverTrends;